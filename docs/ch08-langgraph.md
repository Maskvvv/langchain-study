...
更推荐在生产代码中使用 `interrupt()` 函数配合 checkpointer：
...
- 没有 checkpointer，就无法跨请求恢复。
- `thread_id` 用于定位同一条执行线程。
- `Command(resume=...)` 代表人工输入新的决策后继续执行。
...
前四个示例帮助你理解图、路由、审批和多 Agent，第五个示例更接近真实生产中的暂停与恢复。
